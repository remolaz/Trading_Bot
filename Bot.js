...
.option('-p --product [value]', 'The GDAX product to query', 'BTC-EUR')
.parse(process.argv);
...
const pairs = ["BTC-EUR"];
// const pairs = ["ETH-BTC", "LTC-BTC", "ETH-USD", "BTC-EUR", "LTC-USD"];
//const fromEmail = "btc_eur@gdax.com"
//const toEmail = "remo.tk@gmail.com"
    ...
    feed.on('data', (msg) => {
        if (msg.type == 'myOrderPlaced') {
            ...
            try {
                ...
                // Get the USD amount
                gdax.loadMidMarketPrice(msg.productId.substr(0, 3) + '-EUR').then((price) => {
                    let amt = parseFloat(price)*parseFloat(trade.size);
                    toSend = toSend + ' >> ' + amt.toFixed(2) + 'EUR'
                    console.log(toSend);
